...
function AppContent() {
  ...
  const [hasShownFinalModalThisSession, setHasShownFinalModalThisSession] = useState(() => {
    return sessionStorage.getItem('hasShownFinalModalThisSession') === 'true' || 
           localStorage.getItem('hasShownFinalModalToday') === new Date().toDateString();
  });
  ...
  // Final sınavı modal kontrolü - Her kullanıcıya sadece 1 defa göster
  useEffect(() => {
    const checkFinalExamModal = async () => {
      ...
      try {
        ...
        if (userDoc.exists()) {
          ...
          // Firebase'de hiç gösterilmemişse göster (sadece 0 ise)
          if (shownCount === 0) {
            setTimeout(() => {
              setShowFinalExamModal(true);
              setHasShownFinalModalThisSession(true);
              sessionStorage.setItem('hasShownFinalModalThisSession', 'true');
              localStorage.setItem('hasShownFinalModalToday', new Date().toDateString());
            }, 3000);
          } else {
            // Firebase'de gösterilmişse session ve localStorage'a da kaydet
            setHasShownFinalModalThisSession(true);
            sessionStorage.setItem('hasShownFinalModalThisSession', 'true');
            localStorage.setItem('hasShownFinalModalToday', new Date().toDateString());
          }

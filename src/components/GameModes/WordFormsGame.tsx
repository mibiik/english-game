...
const WordFormsGame: React.FC<WordFormsGameProps> = ({ words }) => {
  ...
  const handleAnswerSubmit = (e?: React.FormEvent) => {
    ...
    if (correct) {
      ...
      if (userId) {
        supabaseGameScoreService.addScore(userId, 'wordForms', totalPoints);
      }
      ...
      const userId = supabaseAuthService.getCurrentUserId();
      if (userId) {
        supabaseGameScoreService.addScore(userId, 'wordForms', -2);
      }
    ...
    if (currentIndex < words.length - 1) {
      ...
      try {
        const userId = supabaseAuthService.getCurrentUserId();
        if (userId) {
          supabaseGameScoreService.addScore(userId, 'wordForms', score);
        }
      } catch (error) {

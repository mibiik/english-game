...
const WordDetective: React.FC<WordDetectiveProps> = ({ words, unit, level }) => {
  ...
  // Sadece cevabın doğru olup olmadığını kontrol et (render için)
  ...
  const selectAnswer = (blankId: number, answer: string) => {
    // Eğer daha önce bu boşluk için cevap verilmişse, skorlama yapma
    if (checkedAnswers[blankId]) {
    ...
    if (isCorrect) {
      // Önce yeni streak'i hesapla
      const newStreak = streak + 1;
      setStreak(newStreak);
      setMaxStreak(prevMax => Math.max(prevMax, newStreak));
      
      // Yeni streak ile skor hesapla
      const points = 10 + ((newStreak - 1) * 2); // İlk doğru 10, sonrası streak bonusu
